 ...
 * @typedef Options
 *   Configuration.
 * @property {RegExp[] | null | undefined} [linkRootToLuoguWhiteList]
 *   URL patterns in list will not point to https://www.luogu.com.cn/,
 *   except /user/${uid} (userMention). (optional).
 * @property {boolean | null | undefined} [userLinkPointToLuogu]
...
export default function remarkLuoguFlavor(options) {
  ...
  return (tree, file) => {
    ...
    visit(tree, "link", (node) => {
      let res = false;
      linkWhiteList.forEach((reg) => {
        res ||= reg.test(node.url);
      });
      if (!res) {
